...
#ifndef _MEX_RELAY_IN_HPP_
#define _MEX_RELAY_IN_HPP_
...
class relay_in : public subport {
    ...
    public:
        relay_in(const char* subport_name, int code, int id):subport(subport_name, id), _code(code){
        ...
        virtual void request(boost::asio::serial_port* bus, json& response){
            ...
            //read status request
            unsigned char frame[] = { 0x01, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
            ...
            //parse data
            bool value = (rbuffer[3]==(unsigned char)_id)?true:false;
            //spdlog::info("{}:{}", _subname, value);
