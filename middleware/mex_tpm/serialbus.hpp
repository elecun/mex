...
class serial {
    ...
    public:
        ...
        /* device port shares to use multiple devices (only for req/res strategy) */
        void add_subport(int idx, subport* dev){
            _subport_container.insert(make_pair<int, subport*>(idx, dev));
        }

        /* remove device port shares */
        void remove_subport(int idx){
            if(_subport_container.find(idx)!=_subport_container.end()){
                _subport_container.erase(idx);
            }
        }
